 ...
 * @since 1.0.0
 */
class TS_Pricing extends Widget_Base {
    use TS_Style_Trait;
    ...
    {
        $this->TS_section_style_controls('section', 'Section', '.ts-el-sec');
        $this->TS_basic_style_controls('pricing_heading_title', 'Title', '.ts-el-title');
        $this->TS_basic_style_controls('pricing_heading_desc', 'List', '.ts-el-list li');

        $this->TS_basic_style_controls('pricing_heading_price', 'Price', '.ts-el-price');
        $this->TS_basic_style_controls('pricing_heading_month', 'Month', '.ts-el-month');
	...
	protected function render() {
		$settings = $this->get_settings_for_display();
            ...
            if ('2' == $settings['TS_btn_link_type']) {
                ...
                $this->add_render_attribute('ts-button-arg', 'class', 'ts-btn-9 ts-btn-12 w-100 ts-el-btn');
            } else {
     ...
     <div class="price__item white-bg mb-30 transition-3 fix p-relative ts-el-sec <?php echo esc_attr($class_name); ?>">
        ...
        <h3 class="price__title ts-el-title"><?php echo TS_kses($settings['title']); ?></h3>
        <?php endif; ?>
        ...
        <div class="price__content">
            ...
            <h4>
                <b><u class="ts-el-list" ><?php echo TS_kses($settings['features_title']); ?></u></b>
            </h4>
            <?php endif; ?>
           <div class="price__list mb-35">
              <ul class="ts-el-list">
              	<?php foreach ($settings['features_list'] as $index => $item) :
              		$availability = $item['TS_feature_unavailable'] ? 'unavailable' : 'price-available';
                ?>
                 <li class="<?php echo esc_attr($availability); ?>"><?php echo TS_kses($item['text']); ?> <span><?php TS_render_icon($item, 'TS_features_icon', 'TS_features_selected_icon'); ?></span></li>
                <?php endforeach; ?>
              </ul>
           </div>
           <?php endif; ?>
			<?php if ($settings['description']) : ?>
                <p class="mb-15"><?php echo TS_kses($settings['description']); ?></p>
            <?php endif; ?>

           <div class="price__amount mb-30">
	            <h4 class="ts-el-price" >
	              	<?php echo esc_html($currency); ?><?php echo TS_kses($settings['price']); ?>
					<?php if ($settings['period']) : ?>
                        <span class="ts-el-month" >/ <?php echo TS_kses($settings['period']); ?></span>
                    <?php endif; ?>
	          	</h4>
           </div>
...
$widgets_manager->register( new TS_Pricing() );