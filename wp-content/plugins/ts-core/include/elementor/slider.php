 ...
 * @since 1.0.0
 */
class TS_Main_Slider extends Widget_Base {
	...
	protected function register_controls_section() {
		...
		$this->start_controls_section(
            'TS_main_slider',
            [
                'label' => esc_html__('Main Slider', 'TScore'),
        ...
        $repeater = new \Elementor\Repeater();
            ...
            $repeater->add_control(
                ...
                [
                    ...
                    'type' => Controls_Manager::TEXT,
                    'default' => esc_html__('Grow business.', 'TScore'),
            ...
            $repeater->add_control(
                'TS_slider_description',
                [
                    'label' => esc_html__('Description', 'TScore'),
                    'description' => TS_get_allowed_html_desc( 'intermediate' ),
                    'type' => Controls_Manager::TEXTAREA,
                    'default' => esc_html__('There are many variations of passages of Lorem Ipsum available but the majority have suffered alteration.', 'TScore'),
                    'placeholder' => esc_html__('Type section description here', 'TScore'),
                ]
            );

			$repeater->add_control(
	            'TS_btn_link_switcher',
	            [
	                'label' => esc_html__( 'Add Button link', 'TScore' ),
	        ...
	        $repeater->add_control(
	            'TS_btn_link',
	            [
	                'label' => esc_html__( 'Button Link link', 'TScore' ),
	        ...
	        $repeater->add_control(
	            'TS_btn_page_link',
	            [
	                'label' => esc_html__( 'Select Button Link Page', 'TScore' ),
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => [
                    [
                        'TS_slider_title' => esc_html__('Grow business.', 'TScore')
                    ],
                    [
                        'TS_slider_title' => esc_html__('Development.', 'TScore')
                    ],
                    [
                        'TS_slider_title' => esc_html__('Marketing.', 'TScore')
                    ],
                ],
                'title_field' => '{{{ TS_slider_title }}}',
            ]
        );
    ...
    protected function style_tab_content()
    {
        ...
        $this->TS_basic_style_controls('slider_title', 'Title', '.ts-el-title');
        $this->TS_basic_style_controls('slider_subtitle', 'Subtitle', '.ts-el-subtitle');
        $this->TS_basic_style_controls('slider_desc', 'Description', '.ts-el-content');
            ...
            <div class="slider__active swiper-container">
               <div class="swiper-wrapper">
           		<?php foreach ($settings['slider_list'] as $item) :
        			$this->add_render_attribute('title_args', 'class', 'slider__title-3 ts-el-title');
					$this->add_render_attribute('title_args', 'data-animation', 'fadeInUp');
					$this->add_render_attribute('title_args', 'data-delay', '.6s');

 					if ( !empty($item['TS_slider_image']['url']) ) {
                        $TS_slider_image_url = !empty($item['TS_slider_image']['id']) ? wp_get_attachment_image_url( $item['TS_slider_image']['id'], $settings['thumbnail_size']) : $item['TS_slider_image']['url'];
                        $TS_slider_image_alt = get_post_meta($item["TS_slider_image"]["id"], "_wp_attachment_image_alt", true);
                    }

					// btn Link
                    if ('2' == $item['TS_btn_link_type']) {
                        $link = get_permalink($item['TS_btn_page_link']);
                        $target = '_self';
                        $rel = 'nofollow';
                    } else {
                        $link = !empty($item['TS_btn_link']['url']) ? $item['TS_btn_link']['url'] : '';
                        $target = !empty($item['TS_btn_link']['is_external']) ? '_blank' : '';
                        $rel = !empty($item['TS_btn_link']['nofollow']) ? 'nofollow' : '';
                    }
                ?>
                <div class="slider__item swiper-slide p-relative slider__height slider__height-3 ts-el-sec d-flex align-items-center z-index-1">
                     <div class="slider__bg slider__overlay slider__overlay-3 include-bg" data-background="<?php echo esc_url($TS_slider_image_url); ?>"></div>
                     <div class="container">
                        <div class="row">
                           <div class="col-xxl-6 col-xl-7 col-lg-8 col-md-10 col-sm-10">
                              <div class="slider__content-3 p-relative z-index-1">
                             	<?php if (!empty($item['TS_slider_sub_title'])) : ?>
                                 <span class="ts-el-subtitle" data-animation="fadeInUp" data-delay=".3s">
                                    <svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="htTS://www.w3.org/2000/svg">
                                    ...
                                    <?php echo TS_kses( $item['TS_slider_sub_title'] ); ?></span>
                                    <?php endif; ?>

								<?php
                                    if ($item['TS_slider_title_tag']) :
                                        printf('<%1$s %2$s>%3$s</%1$s>',
                                            tag_escape($item['TS_slider_title_tag']),
                                            $this->get_render_attribute_string('title_args'),
                                            TS_kses($item['TS_slider_title'])
                                        );
                                    endif;
                                ?>
                                 ...
                                 <div class="slider__search mb-20" data-animation="fadeInUp" data-delay=".9s">
                                    <form action="<?php print eduker_header_search_url();?>">
                                       <div class="slider__search-input p-relative">
                                       	  <input type="search" name="s" value="<?php print esc_attr( get_search_query() )?>" placeholder="<?php echo esc_attr__('Course title here...','TScore'); ?>">
                                          <button type="submit">
                                            <?php echo esc_html__('Search','TScore'); ?>
                                          </button>
                                          <div class="slider__search-input-icon">
                                             <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="htTS://www.w3.org/2000/svg">
                                 ...
                                 <?php endif; ?>
                                 <?php if (!empty($item['TS_slider_description'])) : ?>
                                 <div class="slider__list ts-el-content " data-animation="fadeInUp" data-delay="1.2s">
                                    <?php echo TS_kses( $item['TS_slider_description'] ); ?>
                                 </div>
                                 <?php endif; ?>

                                 <?php if (!empty($link)) : ?>
                                 <div class="slider__btn " data-animation="fadeInUp" data-delay="1.1s">
                                    <a target="<?php echo esc_attr($target); ?>" rel="<?php echo esc_attr($rel); ?>" href="<?php echo esc_url($link); ?>" class="ts-btn-5 ts-btn-11 ts-el-btn">
                                    	<?php echo TS_kses($item['TS_btn_btn_text']); ?>
                                    </a>
                                 </div>
                                 <?php endif; ?>
                              </div>
		...
		?>
         <section class="slider__area">
            <div class="slider__active swiper-container">
               ...
               <div class="swiper-wrapper">
           		<?php foreach ($settings['slider_list'] as $item) :
        			$this->add_render_attribute('title_args', 'class', 'slider__title ts-el-title');
					$this->add_render_attribute('title_args', 'data-animation', 'fadeInUp');
					$this->add_render_attribute('title_args', 'data-delay', '.6s');
                ...
                ?>
                  <div class="slider__item swiper-slide p-relative slider__height ts-el-sec d-flex align-items-center z-index-1">
                     <div class="slider__bg slider__overlay include-bg" data-background="<?php echo esc_url($TS_slider_image_url); ?>"></div>
                     <div class="container">
                        <div class="row">
                           <div class="col-xl-7 col-lg-8 col-md-10 col-sm-10">
                              <div class="slider__content p-relative z-index-1">
                              	<?php if (!empty($item['TS_slider_sub_title'])) : ?>
                                 <span class="ts-el-subtitle" data-animation="fadeInUp" data-delay=".3s"><?php echo TS_kses( $item['TS_slider_sub_title'] ); ?></span>
                             	<?php endif; ?>
                                ...
                                ?>
                                <?php if (!empty($item['TS_slider_description'])) : ?>
                                 <p class="ts-el-content" data-animation="fadeInUp" data-delay=".9s"><?php echo TS_kses( $item['TS_slider_description'] ); ?></p>
                                 <?php endif; ?>

                                 <?php if (!empty($link)) : ?>
                                 <div class="slider__btn" data-animation="fadeInUp" data-delay="1.1s">
                                    <a target="<?php echo esc_attr($target); ?>" rel="<?php echo esc_attr($rel); ?>" href="<?php echo esc_url($link); ?>" class="ts-btn ts-el-btn">
                                    	<?php echo TS_kses($item['TS_btn_btn_text']); ?>
                                    </a>
                                 </div>
                                 <?php endif; ?>
...
$widgets_manager->register( new TS_Main_Slider() );