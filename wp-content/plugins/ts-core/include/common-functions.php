 ...
 * Get All Post Types
 */
function TS_get_post_types()
{

    $TS_cpts = get_post_types(array('public' => true, 'show_in_nav_menus' => true), 'object');
    $TS_exclude_cpts = array('elementor_library', 'attachment');
    foreach ($TS_exclude_cpts as $exclude_cpt) {
        unset($TS_cpts[$exclude_cpt]);
    }
    $post_types = array_merge($TS_cpts);
 ...
 */
function TS_get_all_types_post($post_type)
{
 ...
 * Get all Pages
 */
if (!function_exists('TS_get_all_pages')) {
    function TS_get_all_pages()
    {
 ...
 */
function TS_get_post_settings($settings)
{
 ...
 */
function TS_get_thumbnail_sizes()
{
 ...
 */
function TS_get_orderby_options()
{
    $orderby = array(
        'ID' => 'Post ID',
        'author' => 'Post Author',
 ...
 * Get Post Categories
 */
function TS_get_categories($taxonomy)
{
 ...
 * Get all Pages
 */
if (!function_exists('TS_get_pages')) {
    function TS_get_pages()
    {
 ...
 * @return string
 */
function TS_get_allowed_html_desc($level = 'basic')
{
    if (!in_array($level, ['basic', 'intermediate', 'advance'])) {
        $level = 'basic';
    }

    $tags_str = '<' . implode('>,<', array_keys(TS_get_allowed_html_tags($level))) . '>';
 ...
 * @return array
 */
function TS_get_allowed_html_tags($level = 'basic')
{
    $allowed_html = [
        'b' => [],
        'i' => [
 ...
 * @return bool
 */
if( !function_exists('TS_is_elementor_version')){
    function TS_is_elementor_version($operator = '<', $version = '2.6.0')
    {
 ...
 */
if(!function_exists('TS_render_icon')){
    function TS_render_icon($settings = [], $old_icon_id = 'icon', $new_icon_id = 'selected_icon', $attributes = [])
    {
        ...
        if (TS_is_elementor_version('>=', '2.6.0') && ($is_new || $migrated)) {
 ...
 * Slugify
 */
if (!function_exists('TS_slugify')){
    function TS_slugify($text){
