...
add_filter( 'ocdi/import_files', 'TS_ocdi_import_files' );


function TS_ocdi_page($TS_page_name = 'Home'){
    $posts = get_posts(
        array(
            'post_type'              => 'page',
            'title'                  => $TS_page_name,
...
// after demo imports
function TS_ocdi_after_import_setup( $demo ) {
    $front_page_id = "";
    $blog_page_id = "";
    if( "Home 1" == $demo['import_file_name'] ){
        // Assign front page and posts page (blog page).
        $front_page_id = TS_ocdi_page( 'Home' );
        $blog_page_id  = TS_ocdi_page( 'Blog' );
    }else if( "Home 2" == $demo['import_file_name'] ){
        // Assign front page and posts page (blog page).
        $front_page_id = TS_ocdi_page( 'Home 02' );
        $blog_page_id  = TS_ocdi_page( 'Blog' );
    }
    else if( "Home 3" == $demo['import_file_name'] ){
        // Assign front page and posts page (blog page).
        $front_page_id = TS_ocdi_page( 'Home 03' );
        $blog_page_id  = TS_ocdi_page( 'Blog' );
    }
...
add_action( 'ocdi/after_import', 'TS_ocdi_after_import_setup' );



function TS_ocdi_plugin_page_setup( $default_settings ) {
...
add_filter( 'ocdi/plugin_page_setup', 'TS_ocdi_plugin_page_setup' );